...
| PHASE_07 | 2026-09-06 | IN_PROGRESS | Accounting foundation is implemented: eight schema tables, reversible migration `0004_accounting.sql`, RLS, journal posting/reversal, period close/reopen, trial balance, general ledger, and invariant test coverage. Production sign-off remains pending for sequence-backed numbering, module-lock enforcement, party FKs, and database-level accounting integration tests. See `PHASE_07_IMPLEMENTATION_REPORT.md`.
...
| PHASE_09 | 2026-09-07 | IN_PROGRESS | Inventory ledger foundation plus persisted transfer lifecycle (draft/send/partial receipt/receipt/cancel), approved adjustment deltas, serial reservation, moving-average valuation, recompute, parity helpers, and integration coverage. Remaining acceptance work is live database concurrency proof and complete lot/serial lifecycle endpoints. See `PHASE_09_IMPLEMENTATION_REPORT.md`. |
| PHASE_10 | 2026-09-07 | IN_PROGRESS | Sales invoice foundation with deterministic totals, draft/post/void/payment flows, reference-linked return creation, adjustment-note posting, offer validity/target evaluation, print data, and tenant-scoped routes. Remaining acceptance work is atomic accounting+inventory posting/reversal effects, full return quantity enforcement, and sales integration/concurrency tests. See `PHASE_10_IMPLEMENTATION_REPORT.md`. |
