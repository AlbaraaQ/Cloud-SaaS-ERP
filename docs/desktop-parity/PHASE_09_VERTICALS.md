...
**الحالة: الأجزاء الأول والثاني والثالث والرابع والخامس والسادس مُنجزة** — الجزء الأول 🧑‍💼 المندوبون
...
حيّة، **740** اختبار API). بقية الوحدات — فئات المرسى وأسعارها، سلة — مبدؤها في خارطة
الطريق (`ROADMAP_PHASES_02_11.md`) وقياسها في §10.
...
| السابع | `frmGroupM` | المرسى: 📋 بطاقة الفئة وأسعارها وعروضها و«➕ إضافة مدة» | ⬜ |
| الثامن | `FrmSallah.xaml` · `SallaAPI.cs` · `ManagerOnline.cs` · `OrdersManager.cs` · `ProductsManager.cs` | سلة والمتصلات — حيث «الطلبات» تعني طلبات سلة | ⬜ |
...
* **«🖨️ طباعة» و«🖨️ طباعة»** في `frmBookingM` و`frmViolationM`، وملفّات
  ...
  التقارير.
* **⏮ ◀ ▶ ⏭ تنقّل الصفوف** — أزرار تنقّلٍ على وثيقةٍ واحدة؛ الشبكة والبحث أغنى منها.
* **`Additions` كجدول تعاريف** (الأسماء وأسعارها التي يقرأ منها `cmbAdditions`) — الإضافة
  تُكتب بوصفها وكميتها وسعرها في البطاقة، وتعاريفها تُبنى مع `frmGroupM` في الجزء السابع.
* **`frmOwners` («تعريف مالك»)** — بطاقة عميلٍ كاملة (جوال · هوية · رقم ضريبي · دولة ·
  ...
  (`vessel_owners`) والعميل نفسه يُدار في شاشة العملاء.
