...
export const salesInvoices = pgTable('sales_invoices', {
  ...
  zatcaUuid: uuid('zatca_uuid'), zatcaHash: text('zatca_hash'), zatcaQr: text('zatca_qr'), zatcaStatus: text('zatca_status'), orderType: text('order_type'), tableNo: text('table_no'), combinedInto: uuid('combined_into'), postedAt: timestamp('posted_at', { withTimezone: true }), voidedAt: timestamp('voided_at', { withTimezone: true }),
...
export const salesInvoiceLines = pgTable('sales_invoice_lines', {
  id: uuid('id').primaryKey(), tenantId: uuid('tenant_id').notNull().references(() => tenants.id, { onDelete: 'cascade' }), invoiceId: uuid('invoice_id').notNull().references(() => salesInvoices.id, { onDelete: 'cascade' }), lineNo: integer('line_no').notNull(), itemId: uuid('item_id').references(() => items.id), taxGroupId: uuid('tax_group_id').references(() => taxGroups.id), description: text('description'), modifiers: jsonb('modifiers').$type<Array<Record<string, unknown>>>().notNull().default([]), quantity: numeric('quantity', qty).notNull(), unitPrice: numeric('unit_price', money).notNull(), discountRate: numeric('discount_rate', money).notNull().default('0'), discountAmount: numeric('discount_amount', money).notNull().default('0'), taxRate: numeric('tax_rate', money).notNull().default('0'), net: numeric('net', money).notNull().default('0'), tax: numeric('tax', money).notNull().default('0'), total: numeric('total', money).notNull().default('0'), costTotal: numeric('cost_total', money).notNull().default('0'), metadata: jsonb('metadata').$type<Record<string, unknown>>().notNull().default({}), ...baseAuditColumns(),
...
export const offerItems = pgTable('offer_items', { tenantId: uuid('tenant_id').notNull().references(() => tenants.id, { onDelete: 'cascade' }), offerId: uuid('offer_id').notNull().references(() => offers.id, { onDelete: 'cascade' }), itemId: uuid('item_id').notNull().references(() => items.id), ...baseAuditColumns() }, (t) => ({ key: uniqueIndex('offer_items_offer_item_key').on(t.tenantId, t.offerId, t.itemId) }));
export const offerParties = pgTable('offer_parties', { tenantId: uuid('tenant_id').notNull().references(() => tenants.id, { onDelete: 'cascade' }), offerId: uuid('offer_id').notNull().references(() => offers.id, { onDelete: 'cascade' }), partyId: uuid('party_id').notNull().references(() => parties.id), ...baseAuditColumns() }, (t) => ({ key: uniqueIndex('offer_parties_offer_party_key').on(t.tenantId, t.offerId, t.partyId) }));
