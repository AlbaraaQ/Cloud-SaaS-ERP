...
export const shiftCloses = pgTable('shift_closes', { id: uuid('id').primaryKey(), tenantId: uuid('tenant_id').notNull().references(() => tenants.id, { onDelete: 'cascade' }), branchId: uuid('branch_id').notNull().references(() => branches.id), userId: uuid('user_id').notNull(), openedAt: timestamp('opened_at', { withTimezone: true }).notNull().defaultNow(), closedAt: timestamp('closed_at', { withTimezone: true }), status: text('status').notNull().default('open'), expectedCash: numeric('expected_cash', money).notNull().default('0'), countedCash: numeric('counted_cash', money).notNull().default('0'), diff: numeric('diff', money).notNull().default('0'), summary: jsonb('summary').$type<Record<string, unknown>>().notNull().default({}), reportHtml: text('report_html'), ...baseAuditColumns() }, (t) => ({ open: uniqueIndex('shift_closes_one_open_key').on(t.tenantId, t.branchId, t.userId).where(sql`status = 'open'`), scope: index('shift_closes_scope_idx').on(t.tenantId, t.branchId, t.status) }));
