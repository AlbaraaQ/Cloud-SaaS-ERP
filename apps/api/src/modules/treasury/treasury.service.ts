...
export class TreasuryService {
  ...
  async updateExpenseType(tenantId: string, id: string, input: { nameAr?: string; nameEn?: string | null; accountId?: string; costCenterId?: string | null }) { const [row] = await withTenantTx(this.database.db, tenantId, (tx) => tx.update(expenseTypes).set({ ...(input.nameAr === undefined ? {} : { nameAr: input.nameAr }), ...(input.nameEn === undefined ? {} : { nameEn: input.nameEn }), ...(input.accountId === undefined ? {} : { accountId: input.accountId }), ...(input.costCenterId === undefined ? {} : { costCenterId: input.costCenterId }), updatedAt: new Date() }).where(and(eq(expenseTypes.tenantId, tenantId), eq(expenseTypes.id, id), isNull(expenseTypes.deletedAt))).returning()); if (!row) throw new DomainError('NOT_FOUND', 'Expense card was not found', 404); return row; }
  /** Expense cards are only labels over an account, so removing one never touches the ledger. */
  async deleteExpenseType(tenantId: string, id: string) { const result = await withTenantTx(this.database.db, tenantId, (tx) => tx.update(expenseTypes).set({ deletedAt: new Date(), updatedAt: new Date() }).where(and(eq(expenseTypes.tenantId, tenantId), eq(expenseTypes.id, id), isNull(expenseTypes.deletedAt)))); if (!result.rowCount) throw new DomainError('NOT_FOUND', 'Expense card was not found', 404); return { id, deleted: true }; }
