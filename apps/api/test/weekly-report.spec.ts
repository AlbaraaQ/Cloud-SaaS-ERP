...
describe('weekly platform report (P-C12 deferred delivery)', () => {
  ...
  it('يعرض النافذة المنقضية والموعد القادم قبل أن يُشغَّل التقرير', async () => {
    ...
    // النافذة أسبوعٌ **منقضٍ**: الأحد 2026-09-06 حتى السبت 2026-09-12 — لا الأسبوع الجاري.
    const window = weeklyWindow(wednesday);
    expect(preview.window.start).toBe(window.start.toISOString());
    expect(preview.window.end).toBe(window.end.toISOString());
    expect(preview.window.label).toBe('2026-09-06 → 2026-09-12');
    ...
    expect(preview.variables.week).toBe('2026-09-06 → 2026-09-12');
