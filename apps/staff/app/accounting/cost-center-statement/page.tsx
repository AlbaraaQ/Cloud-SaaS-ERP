 ...
 *
 * `📋 نوع القيد` is deferred (§8): the cloud filters entry types in `القيود اليومية`.
 */
type Row = {
  rank: number;
  date: string;
...
function CostCentreStatement() {
  ...
  const [applied, setApplied] = useState({ centerId: search.get('center') ?? '', accountId: '', branchId: '', from: '', to: '', fullPeriod: true, hidePrevious: false, summary: true });
      ...
      <div className="card tight no-print">
        <div className="row" style={{ flexWrap: 'wrap', alignItems: 'flex-end' }}>
          ...
          <button
            ...
            type="button"
            onClick={() =>
              setApplied({ centerId, accountId, branchId, from, to, fullPeriod, hidePrevious, summary })
            }
          >
            🔍 عرض
          </button>
