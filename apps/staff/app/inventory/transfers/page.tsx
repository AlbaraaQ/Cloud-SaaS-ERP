...
import { arabicName, dateTime, itemLabel, listItems, listWarehouses, quantity, statusLabel, type Item, type Warehouse } from '../../../lib/lookups';
...
export default function TransfersPage() {
  ...
  const [lines, setLines] = useState<Array<{ itemId: string; qtyText: string; unitCostText: string }>>([{ itemId: '', qtyText: '', unitCostText: '' }]);
  ...
  function updateLine(index: number, patch: Partial<{ itemId: string; qtyText: string; unitCostText: string }>) {
  ...
  return (
    <Screen
      title="مناقلة مخزنية"
      subtitle="نقل بضاعة بين مستودعين: مسودة ← إرسال (صرف) ← استلام (إدخال)."
        ...
        <form className="card" onSubmit={createTransfer}>
          ...
          <div className="form-grid">
            <label className="field">
              <span>من مستودع *</span>
              <select className="input" value={fromWarehouseId} onChange={(event) => setFrom(event.target.value)} required>
            ...
            <label className="field">
              ...
              <select className="input" value={toWarehouseId} onChange={(event) => setTo(event.target.value)} required>
                <option value="">— اختر —</option>
                {warehouseRows.filter((row) => row.id !== fromWarehouseId).map((row) => (
                  <option key={row.id} value={row.id}>
                    {arabicName(row)}
                  </option>
                ))}
              ...
              <tbody>
                {lines.map((line, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <input className="input" dir="ltr" inputMode="decimal" value={line.unitCostText} onChange={(event) => updateLine(index, { unitCostText: event.target.value })} />
                    </td>
                    <td>
          ...
          <button className="btn sm" type="button" onClick={() => setLines((current) => [...current, { itemId: '', qtyText: '', unitCostText: '' }])}>
            + سطر
          </button>
        ...
        {(rows) => (
          <DataTable
            ...
            columns={[
              ...
              { key: 'status', header: 'الحالة', cell: (row) => <span className="badge">{TRANSFER_STATUS[row.status] ?? statusLabel(row.status)}</span> },
                ...
                cell: (row) => (
                  <span className="row">
                    {row.status === 'draft' && can('inventory.adjust') && (
                      <button className="btn sm" type="button" onClick={() => act(() => apiPost(`/inventory/transfers/${row.id}/send`, {}), 'تم إرسال المناقلة وخُصمت الكمية من المصدر.')}>
                        إرسال
                      </button>
                    )}
                    {['in_transit', 'partially_received'].includes(row.status) && can('inventory.adjust') && (
                      <button
                        ...
                        onClick={() =>
                          act(
                            () =>
                              apiPost('/inventory/transfers/receive', {
                                transferId: row.id,
                                received: row.lines
                                  .map((line) => ({ lineNo: line.lineNo, qty: String(Number(line.qty) - Number(line.receivedQty)) }))
                                  .filter((line) => Number(line.qty) > 0),
                              }),
                            'تم استلام المناقلة وأُضيفت الكمية للمستودع الهدف.',
                          )
                        }
                      >
                        استلام كامل
                    ...
                    {row.status === 'draft' && can('inventory.adjust') && (
                      <button className="btn sm danger" type="button" onClick={() => act(() => apiPost('/inventory/transfers/cancel', { transferId: row.id }), 'تم إلغاء المناقلة.')}>
                        إلغاء
                      </button>
                    )}
                  </span>
