...
export default function MarinaRotaPage() {
  ...
  return (
    <Screen
      title="خطة الدور"
      subtitle="ترتيب دور المراكب على فترات اليوم. الخطة تنظيمية ولا تنشئ حجزاً ولا فاتورة."
          ...
          <>
            ...
            {rows.map((plan) => (
              <div className="card" key={plan.id}>
                <h3>
                  {plan.name} — {shortDate(plan.planDate)}
                </h3>
                <ol className="kv">
                  {plan.lines.map((line) => (
                    <li key={line.lineNo}>
                      <span>{line.periodLabel ?? '—'}</span>
                      <strong>{vesselName(line.vesselId)}</strong>
                    </li>
                  ))}
                </ol>
              </div>
